...
export default function Home() {
  ...
  }, [essentials, goalAmount, goalYears, monthlyInvesting, savings, stability]);
    ...
    <main>
      ...
      <section className="workspace">
        ...
        <aside className="results">
          ...
          <div className="card result-card"><p className="eyebrow">GOAL FEASIBILITY</p><h3>{goalName} needs <strong>{money(plan.monthlyNeeded)}</strong><small> per month</small></h3><p>{plan.gap > 0 ? `You are short by ${money(plan.gap)} each month at the current timeline.` : "Your current monthly contribution can support this goal under these assumptions."}</p></div>
          <div className="card result-card"><p className="eyebrow">GOAL-WISE PRINCIPLE</p><h3><strong>{plan.ratio[0]}%</strong> stability · <strong>{plan.ratio[1]}%</strong> growth</h3><p>{goalYears <= 3 ? "A fixed, near-term goal generally needs capital protection more than return chasing." : goalYears <= 7 ? "This horizon can balance stability with growth, while keeping the deadline visible." : "A long horizon can tolerate more growth exposure, subject to your risk capacity."}</p></div>
